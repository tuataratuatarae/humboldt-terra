...
humboldt.pnt.index <- function(glob.g, glob.s, sp, R = 100, kern.smooth = 1) {
    ...
    if (ncol(glob.g) == 2) {
        ...
        #swapping 'rgeos:gUnaryUnion' for 'sp::rbind'
        sBsp=as(sf::st_union(st_as_sf(sBsp)), "Spatial")
        sBenv=as(sf::st_union(st_as_sf(sBenv)), "Spatial")
        sBf=sBsp+sBenv
        #swapping 'rgeos::gdifference' for 'sf::st_difference'
       borders = as(as(st_as_sf(sf::st_difference(st_as_sf(sBf),
           
           #modifying this code to be with sf
           sf::st_union(st_as_sf(sBf))
       )), "Spatial"), "SpatialLines")
        # borders = as(vect(st_union(st_as_sf(vect(sBsp)), st_as_sf(vect(sBenv)))), "Spatial")
...
## humboldt.all.except.pnt
```{r}


humboldt.all.except.pnt <- function(inname = "DoItAll", env1, env2, sp1, sp2, rarefy.dist = 0, rarefy.units = "dd", env.reso, 
...
### PNT, line by line
```{r}
#required vars: glob.g, glob.s, sp, R = 100, kern.smooth = 1 
...
R = 100
   ...
   # }
    ...
    # if (ncol(glob.g) == 2) {
        ...
        #swapping 'rgeos::gdifference' for 'terra::erase'
        borders = terra::erase(
           vect(sBf),
           
           #modifying this code to be with sf
           #as(gUnaryUnion(sBf),"SpatialLines"),
           vect(st_union(st_as_sf(vect(sBsp)), st_as_sf(vect(sBenv))))
           )
...
#' NC - the niche truncation index is not working for me with the test data--see the paper to see that the values should be 0.408 and 0.146 respectively
